    ...
    private fun validateAndRegister() {
        ...
        firestore.collection("Registrations").document(registrationId).set(registration)
            .addOnSuccessListener {
                // Also update Realtime Database count (keeping it in sync for the counts shown on other screens)
                event?.id?.let { eId ->
                    database.reference.child("Events").child(eId).child("registeredCount")
                        .setValue((event?.registeredCount ?: 0) + 1)
                    
                    // Add to RTDB Registrations for simple lookups if needed (optional, but good for backward compatibility)
